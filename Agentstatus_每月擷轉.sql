...
BEGIN
    ...
    DECLARE @tableName NVARCHAR(MAX) = N'tblRpt_AgentStatus_' + FORMAT(@lastSixMonth, 'yyyyMM');
    ...
    BEGIN TRY
        ...
        --此表tblRpt_AgentStatus_yyyyMM已存在
        BEGIN 
            -- 開啟 IDENTITY_INSERT
            SET @sql = N'
            ...
            USING (
                ...
                WHERE CONVERT(CHAR(6), StartDatetime, 112) = CONVERT(CHAR(6), @lastSixMonth, 112)
            ) AS Source
            ON Target.DBID = Source.DBID -- 匹配條件
            WHEN NOT MATCHED THEN
                -- 如果子表中有父表沒有插入過的記錄，加入新數據
                INSERT (DBID, AgentID, Status_DBID, Reason_DBID, StartDatetime, callID, entityTypeID, stopReason, TenantID, StatID, Duration, EndDatetime, ScheduledCalcDatetime, ScheduledValidationResult)
                VALUES (Source.DBID, Source.AgentID, Source.Status_DBID, Source.Reason_DBID, Source.StartDatetime, Source.callID, Source.entityTypeID, Source.stopReason, Source.TenantID, Source.StatID, Source.Duration, Source.EndDatetime, Source.ScheduledCalcDatetime, Source.ScheduledValidationResult);
            ...
            SET @sql = N'
            DELETE FROM tblRpt_AgentStatus
            WHERE CONVERT(CHAR(6), StartDatetime, 112) = CONVERT(CHAR(6), @lastSixMonth, 112)
        ...
        END
        ELSE
        --此表tblRpt_AgentStatus_yyyyMM不存在
        BEGIN
            -- 創建表的 SQL（先創建表，再插入資料）
            SET @sql = N'SELECT * INTO ' + QUOTENAME(@tableName) + ' FROM tblRpt_AgentStatus WITH(NOLOCK) WHERE CONVERT(CHAR(6), StartDatetime, 112) = ' + CONVERT(CHAR(6), @lastSixMonth, 112);
